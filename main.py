...
def main():
    ...
    dl, dlt = load_data(SZ)
    opt = torch.optim.AdamW([{"params": m.parameters()}, {"params":class_loss.parameters(), "lr": 3e-4, "betas": (0.9, 0.999)}], lr=lr, betas=(0., 0.99), weight_decay=0.01)
    ...
    all_models.load_state_dict(torch.load("model/ckpt_7900.pth")["model"])
    recipe = TrainAndTest(
